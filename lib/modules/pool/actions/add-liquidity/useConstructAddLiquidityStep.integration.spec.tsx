...
import { AddLiquidityProvider } from './useAddLiquidity'
...
async function testConstructAddLiquidityStep() {
  const { result } = testHook(() => useConstructAddLiquidityStep(humanAmountsIn, poolId), {
    wrapper: Providers,
  })
  return result
}
...
test('TBD', async () => {
  ...
  // await waitFor(() => expect(result.current.step.simulation.error).toBeNull())
